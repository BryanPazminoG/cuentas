...
public class PagoInteresController {
    ...
    public ResponseEntity<List<PagoInteres>> buscarPorCuenta(@PathVariable(name = "cuentaId") Integer cuentaId) {
        log.info("Obteniendo pagos de interés por cuenta: {}", cuentaId);
        try {
            return ResponseEntity.ok(this.pagoInteresService.obtenerPorCuenta(cuentaId));
    ...
    @PostMapping
    public ResponseEntity<Integer> crear(@RequestBody PagoInteresDTO pagoInteres) {
        log.info("Se va a crear el pago de interés: {}", pagoInteres);
        try {
            Integer idPagoInteres = this.pagoInteresService.crear(pagoInteres);
            return ResponseEntity.ok(idPagoInteres);
        } catch(RuntimeException rte) {
